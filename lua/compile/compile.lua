...
local function open_window()
  ...
  if term_winid == nil or vim.fn.getbufinfo(term_bufid)[1].hidden == 1 then
    term_winid = vim.api.nvim_open_win(term_bufid, true, { split = "right", win = 0 })
...
function M.run_cmd(cmd)
  ...
  if term_bufid ~= nil then
    vim.api.nvim_buf_delete(term_bufid, {force=false, unload=false})
  end
  open_window()
  ...
  vim.api.nvim_create_autocmd("TextChanged", { buffer = term_bufid, command = "normal! G$" })
...
function M.stop_cmd()
  ...
  vim.fn.jobstop(jobid)
    vim.notify("Job killed", vim.log.levels.INFO)
end
